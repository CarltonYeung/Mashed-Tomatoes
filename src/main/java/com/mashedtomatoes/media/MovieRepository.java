...
public interface MovieRepository extends CrudRepository<Movie, Long> {
  ...
  @Query(
          value = "SELECT if(rt.mediaID IS NOT Null , avg(rt.score), -1) as avgRating, media.title, media.id, me.boxOffice, me.releaseDate " +
  ...
  @Query(
          value = "Select if(rt.mediaID IS NOT Null , avg(rt.score), -1)as avgRating, md.title, md.id, me.boxOffice, me.releaseDate " +
  ...
  @Query(
          value = "SELECT if(rt.mediaID IS NOT NULL, avg(rt.score), -1) as avgRating, media.title, movie.id, movie.boxOffice, movie.releaseDate " +
  ...
  Page<Object[]> findComingSoonFilms(Pageable pageable, @Param("currentDate") Date currentDate, @Param("timeAhead") Date timeAhead);
  ...
  Page<Object[]> findNowPlayingFilms(Pageable pageable, @Param("currentDate") Date currentDate, @Param("timeBefore") Date timeBefore);

  @Query(
          value = "SELECT if(rt.mediaID IS NOT NULL, avg(rt.score), -1), media.title, movie.id, movie.boxOffice, bpw.year " +
